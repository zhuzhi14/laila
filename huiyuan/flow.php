...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $total["gold_amt"]=$_SESSION["gold_amt"];

    $total["goods_price_formated"]=$total["goods_price"]."金币";
    $total["shipping_fee_formated"]=$total["shipping_fee"]."金币";
    $total["amount_formated"]=$total["amount"]."金币";
...
//print_r("\r\n|".$_CFG['use_integral']."|".$_SESSION['user_id']."|".$user_info['pay_points']."|".$flow_type) ;
    /* 如果使用积分，取得用户可用积分及本订单最多可以使用的积分 */

    $smarty->assign('allow_use_integral', 1);
    $smarty->assign('order_max_integral', flow_available_points());  // 可用金币
    $smarty->assign('your_integral',      $_SESSION["gold_amt"]); // 用户金币
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
        ...
        $total["shipping_fee_formated"]=$total["shipping_fee"]."金币";
        $total["amount_formated"]=$total["amount"]."金币";
        $gold_total=$total["shipping_fee"]+$total["goods_price"];

        $smarty->assign('total', $total);

        $smarty->assign("gold_total",$gold_total);
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
        ...
        $total["goods_price_formated"]=$total["goods_price"]."金币";  $total["shipping_fee_formated"]=$total["shipping_fee"]."金币";       $total["amount_formated"]=$total["amount"]."金币";
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    do
    {
        ...
        $sql = "select order_sn from ". $ecs->table('order_info') ." WHERE order_sn=".$order['order_sn'];
        $row = $db->query($sql);
    } while ($row['order_sn'] == $order['order_sn']) ;

    /* 处理余额、积分、红包 */

    $order['money_paid']=$order["order_amount"];
    $gold_total=$order["order_amount"];
    if($_SESSION["gold_amt"]>=$gold_total){
        if ($order['user_id'] > 0 )
        {
            //print_r("余额支付！") ;
            $msg="金币支付，订单号:".$order["order_sn"];
            $acc=log_account_change($order['user_id'],0, 0, 0, $gold_total * (-1), sprintf($msg),'',$order['order_sn']);
            // var_dump($acc);
            if (!$acc)  {
                die("金币支付失败！");
            }else{
    ...
    $order["user_name"]=$_SESSION["user_name"]."(".$_SESSION["phone"].")";
    ...
    /* 取得支付信息，生成支付代码 */
    if ($order['order_amountorder_amount'] > 0)
    {

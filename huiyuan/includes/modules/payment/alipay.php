...
class alipay
{
    ...
    function get_code($order, $payment)
    {
        ...
        $button = '<div style="text-align:center;border-radius: 10px"><input type="button" onclick="window.open(\'https://mapi.alipay.com/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="' .$GLOBALS['_LANG']['pay_button']. '" /></div>';
    ...
    function respond()
    {
       ...
       // var_dump($order_sn);
        ...
        $num=strpos($_GET['out_trade_no'],'_');
        if($num>0){

            $out=$_GET["out_trade_no"];
            $out_id = explode('_', $out);
            $_GET["out_trade_no"] = $out_id['0'];

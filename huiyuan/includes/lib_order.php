...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        ...
        $order['formated_add_time']       = local_date($GLOBALS['_CFG']['time_format'], $order['add_time']+3600*8);
    }
...
function order_fee($order, $goods, $consignee)
{
    ...
    $total['formated_goods_price']  = $total['goods_price']."金币";
...
function user_info($user_id)
{
    ...
    $user = gc_getuserinfo($user_id) ;
//print_r($user) ;
    /* 格式化帐户余额 */
    if ($user['user_id'] > 0)
    {
        $user['formated_user_money'] = price_format($user['balance'], false);
        //$user['formated_frozen_money'] = price_format($user['frozen_money'], false);
        $user['formated_frozen_money'] = price_format($user['frozen_amt'], false);
        $user['pay_points'] = $user['gold_amt'] ;
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    if ($refund_type ==="1" )
    {
        $ret = false;
        /*
        if ($order['integral_money'] > $amount)
            {
        $ret = log_account_change($user_id, 0, 0, 0, $amount, $change_desc,"",$order["order_sn"]);
            } else {
                $gold_amt=$order['integral_money'] ;
                $balance=$amount - $order['integral_money'];
        $ret = log_account_change($user_id, $balance, 0, 0, $gold_amt, $change_desc,'',$order["order_sn"]);
            }*/
        $change_desc=$change_desc.",订单号:".$order["order_sn"];

        $ret = log_account_change($user_id, $shipping_fee, 0, 0, $amount, $change_desc,"",$order["order_sn"]);
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['subtotal']     = $row['goods_price'] * $row['goods_number'].'金币';
        $row['goods_price']  = $row['goods_price'].'金币';
    ...
    $total['goods_price']  = $total['goods_price']."金币";
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            ...
            /*            $sql = "SELECT ua.*".
                                ...
                                " WHERE u.user_id='$user_id' AND ua.address_id = '$address_id'";
            */

 ...
 * @return  boolen      $bool
 */
function add_order($message)
{
   //首先，会员须存在； 

	if (empty($message['buy_uname']))
	{
		$GLOBALS['err']->add($GLOBALS['_LANG']['buy_title_empty']);
		return false;
	}
	...
	if(empty($user_id))
	{
			$GLOBALS['err']->add($GLOBALS['_LANG']['buy_uid_empty']);
			return false;
	}

	$buy_uname=$message['buy_uname'];
	$buy_num = isset($message['buy_num']) ? intval($message['buy_num']) : 1;
    $shopuser=isset($message['ml_phone'])?$message['ml_phone']:'';
  
	$row = gc_mlbuyorder($buy_uname, $buy_num, $user_id,$message['mltype'],$shopuser) ;
    //var_dump($row);

	if ($row['resp_code'] == '0') {
        @$balance = $row['balance'] ;
        @$gold_amt=$row['gold_amt'] ;
        @$_SESSION['balance'] = $balance ;
        @$_SESSION['gold_amt'] = $gold_amt;
...
{
	  $row = gc_addaccountcheck($surplus,$amount) ;
...
function get_account_log($user_id, $num, $start)
{
    $account_log = array();
		...
		$res = gc_getaccountlist($user_id, "CHK", $start, $num) ;
//		print_r($res) ;
		if ($res['resp_code'] == '0') {
			$cnt = $res['cnt'] ;
			$num = 0 ;
        ...
        $payment_s = isset($rows['payment'])?$rows['payment']:'' ;
            /* 支付方式的ID */
        $sql = 'SELECT pay_id FROM ' .$GLOBALS['ecs']->table('payment').
                   " WHERE pay_name = '$payment_s' AND enabled = 1";
...
{
	$row= gc_delaccountcheck($rec_id,$user_id) ;
	if ($row == null) {
	    return false ;
	}
	if ($row['resp_code'] == '0')
...
{		
		...
		if ($row['user_id'] > 0)
    	return $row['balance']+$row['award_bal'] ;
    else 
    	return 0 ;
}
 ...
 * @return  int
 */
function get_user_drawing($user_id)
{
    $row = gc_getaccountinfo($user_id) ;
    if ($row['user_id'] > 0)
        return $row['balance'] + $row['award_bal'];
 ...
 * @return  int
 */
function get_user_award($user_id)
{		
		$row = gc_getaccountinfo($user_id) ;
//print_r($row) ;		
		if ($row['user_id'] > 0)
    	return $row['award_bal'] ;
    else 
    	return 0 ;
}

function get_user_tb($user_id)
{
    $row = gc_getaccountinfo($user_id) ;
    //print_r($row) ;
    if ($row['user_id'] > 0)
...
function get_user_goldamt($user_id)
{
    $row = gc_getaccountinfo($user_id) ;
    //print_r($row) ;
    if ($row['user_id'] > 0)
 ...
 * @return  int
 */
function get_user_surplusjf($user_id)
{
		$row = gc_getaccountinfo($user_id) ;
		if ($row['user_id'] > 0)
    	return $row['gold_amt'] ;
...
function get_user_default($user_id)
{
    $user_bonus = get_user_bonus();
		...
		$row1= gc_getuserinfo($user_id) ;

		gc_writelog("user_id=".$user_id." gold_amt=".$row['gold_amt']." balance=".$row['balance']."user_name=".$_SESSION['user_name']."|", __FILE__, __LINE__) ;

    $info = array();
    $info['userrealname'] = $_SESSION['user_name'];
    $info['username']  = $_SESSION['user_name']."(".stripslashes($_SESSION['phone'].")");
    $info['shop_name'] = $GLOBALS['_CFG']['shop_name'];
    $info['integral']  = $row['gold_amt']. "M币";
    $info['tb_bal'] = $row['tb_bal'].'T币';
    $info['lb_bal'] = $row['lb_bal'].'L币';
    /* 增加是否开启会员邮件验证开关 */
    if (!isset($row1['reg_email'])) $row1['reg_email']=false ;
    if (!isset($row1['reg_phone'])) $row1['reg_phone']=false ;
    if (!isset($row1['reg_name'])) $row1['reg_name']=false ;
    $info['is_validate'] = ($GLOBALS['_CFG']['member_email_validate'] && !$row1['reg_email'])?0:1;
    $info['reg_phone'] = (!$row1['reg_phone'])?0:1;
    $info['reg_name'] = (!$row1['reg_name'])?0:1;
	$info['credit_line'] = isset($row1['credit_line'])?$row1['credit_line']:"";
    $info['formated_credit_line'] = price_format($info['credit_line'], false);

    //如果$_SESSION中时间无效说明用户是第一次登录。取当前登录时间。
    $last_time = !isset($_SESSION['last_time']) ? $row1['last_time'] : $_SESSION['last_time'];
    ...
    $info['surplus']   = price_format($row['balance']+$row['award_bal'], false);
 ...
 * @return  array
 */
function get_buyorder_list($user_id, $page_size, $start)
{
		$res = gc_mlgetbuylist($user_id,$page_size,$start) ;
		$num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
 ...
 * @return  array
 */
function get_mlorder_list($user_id, $page_size, $start)
{
    $res = gc_mlgetmlorder($user_id,$page_size,$start) ;
    $num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
 ...
 * @return  array
 */
function get_matching_list($user_id, $page_size, $start)
{
    $res = gc_matchinglist($user_id,$page_size,$start) ;
    $num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
 ...
 * @return  array
 */
function get_baoapp_list($user_id, $page_size, $start)
{
    $res = gc_baoapplist($user_id,$page_size,$start,"02") ;
    $num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
    ...
    while ($num < $cnt)
    {
        $row = $res[$num] ;
        $num += 1 ;
        $row['fenshu'] = $row['totalmoney']/100000 ;
        if ($row['sharestype'] == '02') {
            $row['fenhong'] = $row['fenshu']*600 ;
            if ($row['fenshu'] >= 5 && $row['fenshu'] < 10)
                $row['fenhong'] = $row['fenshu']*900 ;
            else if ($row['fenshu'] >= 10 && $row['fenshu'] < 20)
                $row['fenhong'] = $row['fenshu']*1200 ;
            else if ($row['fenshu'] >= 20)
                $row['fenhong'] = $row['fenshu']*1500 ;
        }        
        $row['formated_add_time'] = local_date($GLOBALS['_CFG']['date_format'], $row['addtime']);
 ...
 * @return  array
 */
function get_buy_list($user_id, $page_size, $start)
{
		$res = gc_mlgetlist($user_id,$page_size,$start) ;
		$num = 0;
    $bcomments = array();
 ...
 * @return  array
 */
function get_fenhong_list($user_id, $page_size, $start)
{
		$res = gc_mlfenhonglist($user_id,$page_size,$start) ;
		$num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
    ...
    while ($num < $cnt)
    {
    		...
    		$num += 1 ;    		
        $row['formated_add_time'] = local_date($GLOBALS['_CFG']['date_format'], $row['addtime']);
 		 		$row['stardatestr'] = local_date($GLOBALS['_CFG']['date_format'], $row['stardate']);
		 		$row['enddatestr'] = local_date($GLOBALS['_CFG']['date_format'], $row['enddate']);
		 		$uid=$row['uid'];
		 		$row['user_realname'] = $row['user_name'] ;
 ...
 * @return  array
 */
function get_myreg_list($user_id, $page_size, $start)
{
    $sql = "SELECT *  FROM " . $GLOBALS['ecs']->table('users') . "  WHERE baouid='$user_id'  order by user_id desc ";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $row['formated_reg_time'] = local_date($GLOBALS['_CFG']['date_format'], $row['reg_time']);
  	     $rank_id=$row['user_rank'];
 		 $row['user_rankname'] =$GLOBALS['db']->getOne("SELECT rank_name FROM " . $GLOBALS['ecs']->table('user_rank') . " where rank_id=".$rank_id);
	    $uid=$row['user_id'];
		 $row['user_realname'] =$GLOBALS['db']->getOne("SELECT content FROM " . $GLOBALS['ecs']->table('reg_extend_info') . " where reg_field_id=101 and user_id =".$uid);
		 $bcomments[] = $row;
    }
 ...
 * @return  array
 */
function get_myrecom_list($user_id, $page_size, $start)
{
    $sql = "SELECT *  FROM " . $GLOBALS['ecs']->table('users') . "  WHERE reid='$user_id'  order by user_id desc ";
    $res = $GLOBALS['db']->SelectLimit($sql, $page_size, $start);
 ...
 *
 * */
function  insert_maila($message){


    $sql="INSERT INTO " .$GLOBALS['ecs']->table('maila').
        "(id, bordernum, whopay, mltype, mert_user_id, buy_uname, buy_method, buy_num, buy_amt,is_pay)  VALUES ('','$message[bordernum]','$message[whopay]','$message[mltype]','$message[mert_user_id]','$message[buy_uname]','$message[buy_method]','$message[buy_num]','$message[buy_amt]',0)";
...
function  update_maila_pay($is_pay,$bodernum){

    $sql="UPDATE".$GLOBALS['ecs']->table('maila').'SET `is_pay`='.$is_pay.' where  bordernum='.$bodernum;
...
function  select_maila($order_sn){

    $sql="select * from ".$GLOBALS['ecs']->table('maila').'where `bordernum`='.$order_sn;

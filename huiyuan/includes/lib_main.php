...
{
		gc_writelog("begin:update_user_info!".$_SESSION['user_id']." user_name".$_SESSION['user_name'], __FILE__, __LINE__) ;
    if (!$_SESSION['user_id'])
    {
        return false;
    }

		$row = gc_getuserinfo($_SESSION['user_id']) ;
    if ($row['user_id'] > 0)
    {
        /* 更新SESSION */
//        $_SESSION['last_time']   = $row['last_login'];
        ...
        $_SESSION['email']       = $row['email'] ;
				...
				gc_writelog("!! user_name=".$_SESSION['user_name']."user_rank".$_SESSION['user_rank'], __FILE__, __LINE__) ;
		}
}
...
function get_user_info($id=0){
	...
	//	var_dump($_SESSION);
   // var_dump($id);
		$row = gc_getaccountinfo($id);

		if ($row['user_id'] > 0) {
			$user['userrealname']=$_SESSION['user_name'] ;
	    $user['username']  = $_SESSION['user_name'] ;
	    $user['phone']    = @$_SESSION['phone'] ;
	    $user['user_points'] = $row['gold_amt'] . $GLOBALS['_CFG']['integral_name'];
	    $user['user_money']  = price_format($row['balance'], false);
	    $user['award_bal']  = price_format($row['award_bal'], false);
	    $user['user_bonus']  = price_format($row['award_bal'], false);
	  }
...
/* --update by guo--   $ur_here    = '<a href=".">' . $GLOBALS['_LANG']['home'] . '</a>';*/
    $ur_here    = '<a href=".">首页</a>';
 ...
 * @return  array       $pager
 */
function get_pager($url, $param, $record_count, $page = 1, $size = 10,$do=1)
{
    $size = intval($size);
    if ($size < 1)
    {
  ...
  if($do===1) {
      ...
      if ($pager['styleid'] == 0) {
          $pager['page_first'] = $url . $param_url . 'mpage=1';
          $pager['page_prev'] = $url . $param_url . 'mpage=' . $page_prev;
          $pager['page_next'] = $url . $param_url . 'mpage=' . $page_next;
          $pager['page_last'] = $url . $param_url . 'mpage=' . $page_count;
          $pager['array'] = array();
          for ($i = 1; $i <= $page_count; $i++) {
              $pager['array'][$i] = $i;
          }
          ...
          $url_format = $url . $param_url . 'mpage=';
          $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $url_format . 1 : '';
          $pager['page_prev'] = ($page > 1) ? $url_format . $page_prev : '';
          $pager['page_next'] = ($page < $page_count) ? $url_format . $page_next : '';
          $pager['page_last'] = ($_to < $page_count) ? $url_format . $page_count : '';
...
//delete by guo    $smarty->assign('copyright',     sprintf($GLOBALS['_LANG']['copyright'], date('Y'), $GLOBALS['_CFG']['shop_name']));

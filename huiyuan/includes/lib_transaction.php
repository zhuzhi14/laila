...
//    $info['rank_points'] = isset($infos['rank_points']) ? $infos['rank_points'] : '';
    $info['pay_points']  = isset($infos['gold_amt'])  ? $infos['gold_amt']  : 0;
    $info['user_money']  = isset($infos['balance'])  ? $infos['balance']  : 0;
...
function cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql))
    {
        ...
        if ($order['user_id'] > 0 && $order['surplus'] > 0)
        {
            ...
            $accok = log_account_change($order['user_id'], $order['surplus'], 0, 0, 0, $change_desc);
        }
        if ($order['user_id'] > 0 && $order['integral'] > 0)
        {
            $change_desc = sprintf($GLOBALS['_LANG']['return_integral_on_cancel'], $order['order_sn']);
            $accok = log_account_change($order['user_id'], 0, 0, 0, $order['integral'], $change_desc);
        }
        if (!$accok) {
        	return false ;
        }
...
function save_consignee($consignee, $default=false)
{
    ...
    {
        /* 保存为用户的默认收货地址 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('user_address') .
            " SET activity = '0' WHERE user_id = '$consignee[user_id]'";

        $res = $GLOBALS['db']->query($sql);

        $sql = "UPDATE " . $GLOBALS['ecs']->table('user_address') .
            " SET activity = '1' WHERE user_id = '$consignee[user_id]' and address_id = '$consignee[address_id]'";
...
function get_address_id($user_id)
{
		$sql = "select address_id FROM " .$GLOBALS['ecs']->table('user_address') . " WHERE user_id = '$user_id' and activity = '1'";
		$res = $GLOBALS['db']->query($sql);
		if (!$res)  {
			$sql = "select max(address_id) FROM " .$GLOBALS['ecs']->table('user_address') . " WHERE user_id = '$user_id'";
			$res = $GLOBALS['db']->query($sql);	
		}
		return $res ;
}
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            $payment_info['pay_code']="alipay_buy";
            /* 调用相应的支付方式文件 */

...
//    print_r($GLOBALS['_CFG']['integrate_code']) ;
//    include_once(ROOT_PATH . 'includes/modules/integrates/' . $GLOBALS['_CFG']['integrate_code'] . '.php');
    include_once(ROOT_PATH . 'includes/modules/integrates/tdwsys.php');
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        {
            $arr['integrate_code'] = 'tdwsys'; // 默认的会员整合插件为 ecshop
        }
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '',$change_type, $bordernum = '')
{
        if ($user_money==0 && $pay_points == 0 && $frozen_money==0) 
            return null;
        
		$row = gc_account_change($user_id, $user_money, $frozen_money, $pay_points, $change_desc, '030000', $bordernum) ;
        //var_dump($row);
		return $row;
}

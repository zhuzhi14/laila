...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points,recode,user_rank, reg_time ".
    ...
    $rank_id=$user_list[$i]['user_rank'];
 		$user_list[$i]['user_rankname'] = $GLOBALS['db']->getOne("SELECT rank_name FROM " . $GLOBALS['ecs']->table('user_rank') . " where rank_id=".$rank_id);
	}

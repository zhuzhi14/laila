...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('money' == $step)
    {
        ...
        if ($old_order['user_id'] > 0)
        {
            ...
            {
                log_account_change($user['user_id'], 0, 0, 0, $user_money_change, sprintf($_LANG['change_use_surplus'], $old_order['order_sn']),'',$order['order_sn']);
            }

            $pay_points_change = $old_order['integral'] - $order['integral'];
            if ($pay_points_change != 0)
            {
                log_account_change($user['user_id'], 0, 0, 0, $pay_points_change, sprintf($_LANG['change_use_integral'], $old_order['order_sn']),'',$order['order_sn]']);
            }
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    /* 取得满足条件的订单 */
    $sql = "SELECT order_sn, user_name " .
        "FROM " . $ecs->table('order_info') .
        "WHERE user_id > 0 " .
        "AND extension_code = '' " . order_query_sql('unprocessed');
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['cancel']))
    {
        ...
        if($order["money_paid"]=="0.00"){
            $order["money_paid"]=$order["total_fee"];
        }
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        $user_info=gc_getuserinfo($order["user_id"]);

        $gold_amt = $user_info['gold_amt'];
        //操作留言
        $admin_msg=$_REQUEST["action_note"].",订单号：".$order['order_sn'];


        $order_amount = $order["order_amount"];


        if($gold_amt > 0 && $order["user_id"] > 0){
            if($gold_amt>=$order_amount){

                log_account_change($order['user_id'], 0, 0, 0, (-1)*$order_amount, sprintf($admin_msg),'',$order['order_sn']);
    ...
    elseif ('unship' == $operation)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            /* 计算并退回积分 */
            $integral = integral_to_give($order);
            log_account_change($order['user_id'], 0, 0, (-1) * intval($integral['rank_points']), (-1) * intval($integral['custom_points']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']),'',$order['order_sn']);
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        log_account_change($order['user_id'], 0, 0, 0, $surplus, sprintf($GLOBALS['_LANG']['return_order_surplus'], $order['order_sn']),'',$order['order_sn']);
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
    }

    if ($order['user_id'] > 0 && $order['integral'] > 0)
    {
        log_account_change($order['user_id'], 0, 0, 0, $order['integral'], sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn']),'',$order['order_sn']);
    }
    ...
    if ($result === false)
    {
        ...
        */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id, " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
            "IFNULL(o.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".

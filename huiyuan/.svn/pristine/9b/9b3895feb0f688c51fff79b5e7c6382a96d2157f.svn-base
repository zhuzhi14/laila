...
/*初始化数据交换对象 */
$exc = new exchange($ecs->table("fenhong"), $db, 'id', 'ucode');
...
if ($_REQUEST['act']=='list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['101_user_fenhong']);
    $smarty->assign('full_page',    1);
    $smarty->display('user_fenhong.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_fenhong.htm'), '', array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax 删除留言
/*------------------------------------------------------ */
...
function msg_list()
{
    ...
    if ($filter['keywords'])
    {
        $where .= " AND (ucode LIKE '%" . mysql_like_quote($filter['keywords']) . "%' or from_onum LIKE '%" . mysql_like_quote($filter['keywords']) . "%') ";
    }
   

    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('fenhong') ." WHERE id >0 " ;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    /* 分页大小 */
    $filter = page_and_size($filter);

    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('fenhong') . "  WHERE  id > 0 $where  ORDER by $filter[sort_by] $filter[sort_order] ".
            "LIMIT " . $filter['start'] . ', ' . $filter['page_size'];

    $msg_list = $GLOBALS['db']->getAll($sql);
    foreach ($msg_list AS $key => $value)
    {    
        $msg_list[$key]['addtime'] = local_date($GLOBALS['_CFG']['time_format'], $value['addtime']);
		$msg_list[$key]['stardate'] = local_date($GLOBALS['_CFG']['time_format'], $value['stardate']);
		$msg_list[$key]['enddate'] = local_date($GLOBALS['_CFG']['time_format'], $value['enddate']);

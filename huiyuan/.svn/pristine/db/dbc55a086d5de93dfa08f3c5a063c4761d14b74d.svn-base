...
/* 权限判断 */
admin_priv('user_buyorder');
/*初始化数据交换对象 */
$exc = new exchange($ecs->table("buyorder"), $db, 'o_id', 'bordernum');
...
if ($_REQUEST['act']=='list')
{
    ...
    $smarty->assign('buyorder_list',     $msg_list['msg_list']);
    ...
    $smarty->assign('ur_here',      $_LANG['100_user_buyorder']);
    $smarty->assign('full_page',    1);
    $smarty->display('user_buyorder.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_buyorder.htm'), '', array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax 删除留言
/*------------------------------------------------------ */
...
function msg_list()
{
    ...
    $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'o_id' : trim($_REQUEST['sort_by']);
    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

    $where = '';
    if ($filter['keywords'])
    {
        $where .= " AND ucode LIKE '%" . mysql_like_quote($filter['keywords']) . "%' ";
    }
    if ($filter['msg_type'] != -1)
    {
        $where .= " AND ifout = '$filter[msg_type]' ";
    }

    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('buyorder'). " WHERE o_id>0   " . $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    /* 分页大小 */
    $filter = page_and_size($filter);

    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('buyorder'). "WHERE o_id>0 $where   ORDER by $filter[sort_by] $filter[sort_order] ".
            "LIMIT " . $filter['start'] . ', ' . $filter['page_size'];

    $msg_list = $GLOBALS['db']->getAll($sql);
    foreach ($msg_list AS $key => $value)
    {    
        $msg_list[$key]['buy_time'] = local_date($GLOBALS['_CFG']['time_format'], $value['addtime']);
        $msg_list[$key]['buy_type'] = $GLOBALS['_LANG']['states'][$value['ifout']];
    }

 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: usercenter.php 16643 2009-09-08 07:02:13Z liubo $
 */
...
$ui_arr = array('register','in_register','login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list','gpos',
    ...
    'account_deposit', 'account_log','account_list','accountjf_list', 'account_detail','account_detailjf', 'act_account', 'pay', 'default', 'bonus', 
    'group_buy', 'group_buy_detail', 'affiliate', 'afftree','stock_trade','comment_list','upvip','fhlist','myreg','myrecom','buy_list','account_list',
    'upbaodan','in_register','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','safe_center',
    'true_info','act_add_card','ml100','mlorderlist');
...
$md_jb=isset($_SESSION['md_jb'])?$_SESSION['md_jb']:"0" ;
$urankname = "" ;
//$myrank=4;
...
//print_r($action) ;
gc_writelog("B=".$user_id."|".$urankname."|".$mybranch."|", __FILE__, __LINE__) ;
...
if ($action == 'do_login')
{
    ...
    {
        ecs_header("Location:usercenter.php\n");
        $login_faild = 1;
    }
    else
    {
            ...
            if($gourl){
               ...
               // break;
                ...
                if ($db->getOne($sql) > 0){
                    ...
                    ecs_header("Location:usercenter.php?act=llcenter\n");
                    exit;
                }
            }
        }
...
{
   $disphard='1';
   wxGetOpenId($disphard) ;
   $openid = isset($_COOKIE['openid'])?$_COOKIE['openid']:null ;
   wechat_bind_list($openid);
}
/* 微信绑定账号列表*/
elseif ($action == 'wechat_ll_list')
{
   $disphard='2';
   wxGetOpenId($disphard) ;
   $openid = isset($_COOKIE['openid'])?$_COOKIE['openid']:null ;
...
elseif ($action == 'bind_wechat')
{
    ...
    if (empty($user_name) || empty($pwd))
    {
        ecs_header("Location:usercenter.php?act=bind\n");
        $login_faild = 1;
    }
    else
    {
...
elseif ($action == 'order_list')
{
    ...
    if ($record_count > 0){
        ...
        $pagebar = get_wap_pager($record_count, $page_num, $page, 'usercenter.php?act=order_list', 'page');
...
elseif($action=='order_info'){
    ...
    if (empty($goods_list))
    {
        $tips = '<br><br>无效错误订单<br><br><a href=usercenter.php?act=order_list class=red>返回我的订单</a>';
...
elseif ($action == 'cancel_order')
{
    ...
    {
        ecs_header("Location: usercenter.php?act=order_list\n");
        exit;
    }
}
...
elseif ($action == 'act_edit_profile')
{
    ...
    if ($res['resp_code']=== 0)
    {
        echo"<SCRIPT LANGUAGE='javascript'>alert('".$_LANG['edit_profile_success']."');location.href='usercenter.php';</SCRIPT>";
    }
    else
    {
...
elseif ($action == 'act_edit_password')
{
    ...
    else
    {
        ...
        echo "<script>alert('修改失败');window.location.href='usercenter.php?act=safe_center';</script>";
        // $f = 4;
...
elseif ($action == 'logout')
{
   ...
   // $smarty->assign('footer', @get_footer());
    ...
    if ($res['resp_code'] === 0) {
        ...
        echo '<script language=javascript>alert("密码修改成功");window.location.href="usercenter.php?act=forgetpwd";</script>';

    } else {
        $rspmsg = isset($res["resp_msg"])?$res["resp_msg"]:"密码修改失败!" ;

        echo '<script language=javascript>alert("'.$rspmsg.'");window.location.href="usercenter.php?act=forgetpwd";</script>';
    }
...
elseif ($action == 'register')
{
    ...
    {
        $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'usercenter.php') ? './index.php' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    }
    /* 取出注册扩展字段 */
...
elseif ($action == 'add_address')
{
    ...
    $smarty->assign('action', "usercenter.php?act=add_edit_address");
...
elseif ($action == 'address_list')
{
    ...
    $smarty->assign('action', "usercenter.php?act=act_edit_address");
...
elseif ($action == 'act_edit_address'){
    ...
    if($result){
        echo '<script language=javascript>alert("修改收货地址成功");location.href="usercenter.php?act=address_list";</script>';
    }
    else{
...
elseif ($action == 'add_edit_address'){
    ...
    $result = update_address($consignee);
    if($result){
        echo '<script language=javascript>alert("增加收货地址成功");location.href="usercenter.php?act=address_list";</script>';
    }
    else{
...
elseif ($action == 'drop_address')
{
    ...
    {
        ecs_header("Location: usercenter.php?act=address_list\n");
        exit;
    }
}
/* 添加收藏商品(ajax) */
...
elseif ($action == 'upvip')
{
    ...
    $pager = get_pager('usercenter.php', array('act' => $action), $recode_count, $page, 10);
...
elseif ($action == 'mlorderlist')
{
    ...
    $pager = get_pager('usercenter.php', array('act' => $action), $recode_count, $page, 20);

    $res = gc_mlgetmlorder($user_id,$pager['size'],$pager['start']) ;
    $num = 0;
    $bcomments = array();
    $cnt = $res['cnt'] ;
    $pwnum = $res['maxint'] - $res['myint'] ;
    if ($pwnum < 0) {
        $pwsm='您现在排第一位!' ;
    } else {
        $pwsm = "您再增加".($res['maxint'] - $res['myint'] + 1).'即排第一位!' ;
    } 
    while ($num < $cnt)
    {
        $row = $res[$num] ;
        $num += 1 ;
        $row['formated_add_time'] = local_date($GLOBALS['_CFG']['date_format'], $row['addtime']);
        $bcomments[] = $row;
    }
        
    $smarty->assign('user_name', $user_name) ;
    $smarty->assign('pwsm', $pwsm) ;
    $smarty->assign('mlorder_list', $bcomments);
    $smarty->assign('pager',        $pager);
    $smarty->display('mala.dwt');
}
/* 添加我的购单 */
elseif ($action == 'act_add_order')
{
    ...
    {
        show_message($_LANG['add_buy_success'], $_LANG['buy_list_lnk'], 'usercenter.php?act=upvip&order_id=' . $message['bordernum'],'info');
//        $err->show($_LANG['add_buy_success'], 'usercenter.php?act=upvip');
    }
    else
    {
        $err->show($_LANG['buy_list_lnk'], 'usercenter.php?act=upvip');
    }
}
...
elseif ($action == 'fhlist')
{
    ...
    $pager = get_pager('usercenter.php', array('act' => $action), $record_count, $page, 5);
...
elseif ($action == 'myreg')
{
    ...
    $record_count = $db->getOne($sql);
    $pager = get_pager('usercenter.php', array('act' => $action), $record_count, $page, 20);
...
elseif ($action == 'buy_list')
{
    ...
    $pager = get_pager('usercenter.php', array('act' => $actionall), $record_count, $page, 10);
...
elseif ($action == 'act_add_baodan')
{
   ...
   // print_r($buy_num) ;
    ...
    if ($cc == null) {
        show_message("购买失败，稍后再操作！", $_LANG['app_list_lnk'], 'usercenter.php?act=upbaodan','info');
    }

    if ($cc['resp_code'] === 0)
    {
        show_message($_LANG['add_app_success'], $_LANG['app_list_lnk'], 'usercenter.php?act=upbaodan','info');
    ...
    {
        show_message($cc['resp_msg'], $_LANG['app_list_lnk'], 'usercenter.php?act=upbaodan','info');
    }
}
...
elseif ($action == 'afftree')
{
    ...
    while ($cnt >= 20) {
        ...
        $pagebar = get_wap_pager($cnt, $page_num, $page, 'usercenter.php?act=afftree', 'page');
...
elseif ($action == 'stocks_cancel') {
    ...
    if ($ret['resp_code'] === 0) {
        show_message($ret['resp_msg'], $_LANG['app_list_lnk'], 'usercenter.php?act=stock_trade','info');
    } else {
        show_message($ret['resp_msg'], $_LANG['app_list_lnk'], 'usercenter.php?act=stock_trade','info');
    }
}
...
//print_r($res) ;
    if (!empty($res)) {
        if ($res['resp_code'] === 0) {
            show_message("成功提交领取申请!", $_LANG['app_list_lnk'], 'usercenter.php?act=gpos','info');
        } else {
            show_message("提交申请失败", $_LANG['app_list_lnk'], 'usercenter.php?act=gpos','info');
        }
    } else {
        show_message('提交申请失败！', $_LANG['app_list_lnk'], 'usercenter.php?act=gpos','info');
    }
...
//  var_dump($type);
    ...
    if ($row == null)  {
        show_message("重置密码出错", "安全中心", 'usercenter.php?act=safe_center');
    }
    if ($row['resp_code'] === 0) {
        show_message($row['resp_msg'], "安全中心", 'usercenter.php?act=safe_center');
    } else {
        show_message("重置密码失败", "安全中心", 'usercenter.php?act=safe_center');
    }
    ...
    if($old_pwd==$new_pwd1){
        ...
        show_message("旧密码与新密码一致！", "安全中心", 'usercenter.php?act=safe_center');

    }
    if ($new_pwd1 != $new_pwd2) {
        show_message("二次输入新密码不一致！", "安全中心", 'usercenter.php?act=safe_center');
    }
    if (strlen($old_pwd) != 6 || strlen($new_pwd1) != 6 || strlen($new_pwd2) != 6) {
        show_message("密码长度必须6位数字！", "安全中心", 'usercenter.php?act=safe_center');
    }
    if (is_numeric($old_pwd) == false || is_numeric($new_pwd1) == false || is_numeric($new_pwd2) == false) {
        show_message("密码必须输入全部数字！", "安全中心", 'usercenter.php?act=safe_center');
    }
    ...
    if ($row == null)  {
        show_message("修改密码出错", "安全中心", 'usercenter.php?act=safe_center');
    }
    if ($row['resp_code'] === 0) {
        show_message($row['resp_msg'], "安全中心", 'usercenter.php?act=safe_center');
    } else {
...
elseif ($action == 'get_user_name') {
    ...
    $realname['user_id']=$_SESSION["user_id"];
...
//    print_r($row) ;
    if ($row == null)  {
        show_message("充值出错", "金币充值", 'usercenter.php?act=goldcard');
    }
    if ($row['resp_code'] === 0) {
        show_message($row['resp_msg'], "金币充值", 'usercenter.php?act=goldcard');
    } else {
        show_message("出错:".$row['resp_msg'], "金币充值", 'usercenter.php?act=goldcard');
    }
...
else if ($action=="mlmenu")
{
    ...
    $_SESSION['disphard'] = $disphard ;
    $smarty->assign('disphard', $disphard);
    $smarty->display('mlmenu.dwt');
    exit ;
}
else if ($action=="mlcenter") 
{
    $disphard = '1' ;
    wxReLogin($disphard) ;    
    $smarty->assign('disphard', $disphard);
...
//print_r("show_user_center") ;
   ...
   // var_dump($user_id);
    $res_info=gc_getuserinfo($user_id);
     $myrank=@$res_info['user_rank'];
     $mybuss=@$res_info['buss_rank'];
     $mybranch=@$res_info['branch_rank'];
    ...
    @$urankname=@get_rank($myrank,$mybuss,$mybranch);
   // var_dump($urankname);
   ...
   $GLOBALS['smarty']->assign('rank_name', @$urankname);
    ...
    $GLOBALS['smarty']->display('usercenter.dwt');
}
...
//		window.location.href=\"usercenter.php?act=register\";
        //注册失败
        return false;
    }
    else
    {
        ...
        echo "<script>window.location.href=\"usercenter.php?act=default\"; </script>";
        return true;
    }
...
function wxGetOpenId($disphard)
{
    $olddisphard = isset($_SESSION['disphard'])?$_SESSION['disphard']:'0' ;
    if ($olddisphard != $disphard) {
        $_SESSION['disphard'] = $disphard ;
        gc_get_openid() ;
    } else {
        $openid = isset($_COOKIE['openid'])?$_COOKIE['openid']:null ;
...
function m_in_register($phone, $pidno, $password,$username, $email,$recode)
{
    ...
    if ($row['user_id'] === 0)
    {
        $ret = $row['resp_msg'];
        echo "<script>alert('".$ret."');history.go(-1); </script>";
        //		window.location.href=\"usercenter.php?act=in_register\";
        //注册失败
        return false;
    }
    else
    {
        //注册成功
        echo "<script>alert('注册成功!');window.location.href=\"usercenter.php?act=in_register\"; </script>";
        return true;
    }
}
...
function  get_rank($myrank,$mybuss,$mybranch){

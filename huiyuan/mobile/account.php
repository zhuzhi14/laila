...
/*
	if($record_count){
		...
		//获取余额记录
		$account_log = get_account_log($user_id, $page_num, $page_num * ($page - 1));		
		$smarty->assign('account_log', $account_log);
		gc_writelog($account_log) ;
	}
*/
...
elseif ($act == 'account_detail')
{
    ...
    $res = gc_getaccountlist($user_id, "ALL", $pager['start'], $pager['size']) ;
    if ($res['resp_code'] == '0') {
        $cnt = $res['cnt'] ;
...
/*	
	if ($result)
	{
        show_message("删除成功", "返回操作界面", 'account.php?act=account_log','info');
	} else {
	    show_message("删除成功", "返回操作界面", 'account.php?act=account_log','info');
	}
*/	
...
elseif ($act == 'account_credit')
{
	...
	$smarty->assign('act', 'account_credit');
	$smarty->display('account.dwt');
}
/* 会员预付款界面 */
elseif ($act == 'account_award')
{
	include_once(ROOT_PATH . 'includes/lib_clips.php');
	...
	$smarty->assign('myrealname',   $_SESSION['user_name']);
	$smarty->assign('act', 'account_award');
	$smarty->display('account.dwt');
}
/* 会员预付款界面 */
elseif ($act == 'account_goldtransfer')
{
	include_once(ROOT_PATH . 'includes/lib_clips.php');
	...
	$smarty->assign('myrealname',   $_SESSION['user_name']);
	$smarty->assign('act', 'account_goldtransfer');
	$smarty->display('account.dwt');
}
/* 对会员余额申请的处理 */
elseif ($act == 'act_account')
{
	...
	if ($amount <= 0)
	{
        show_message("请在“金额”栏输入大于0的数字", "返回操作界面", 'account.php?act=account_credit','info');
        return 0 ;
	}
	//获取剩余余额
	$surplus_amount = get_user_surplus($user_id);
	...
	if ($surplus['process_type'] == 1)
	{
		...
		{
            show_message("您要申请提现的金额超过了您现有的余额，此操作将不可进行！", "返回操作界面", 'account.php?act=account_credit','info');
            return 0;
		}

        if ($_SESSION['realnameflag'] == 1) {
            if ($amount < 100) {
                show_message("提款金额太小，请输入大于100元！", "返回操作界面", 'account.php?act=account_credit','info');
            }
            ...
            if ($row == null)
            {
               show_message("此次操作失败，请返回重试！", "返回操作界面", 'account.php?act=account_credit','info');
            } else  if ($row['resp_code'] !== 0) {
               show_message('出错:'.$row['resp_msg'], "返回操作界面", 'account.php?act=account_credit','info');
            } else {
               show_message("您的提现申请已成功提交!", "返回操作界面", 'account.php?act=account_credit','info');
            }
        } else {
            //插入会员账目明细
    		$amount = '-'.$amount;
    		...
    		if ($surplus['rec_id'] > 0)
    		{
                show_message("您的提现申请已成功提交，请等待管理员的审核", "返回操作界面", 'account.php?act=account_credit','info');
    		}
    		else
    		{
                show_message("此次操作失败，请返回重试！", "返回操作界面", 'account.php?act=account_credit','info');
    ...
    else if ($surplus['process_type'] == 2)
    {
        ...
        if ($amount > $sur_amount)
        {
            show_message("您的转现的余额不足！", "返回操作界面", 'account.php?act=account_award','info');
            return 0;
        }
        ...
        if ($ret == null)
        {
            show_message("您的转现交易失败 ！", "返回操作界面", 'account.php?act=account_award','info');
        }
        else if ($ret['resp_code'] === 0) 
        {
            show_message("您的转现交易成功 ！", "返回操作界面", 'account.php?act=account_award','info');
        } else {
            $content = '金币转账失败：'.$ret['resp_msg'] ;
            show_message($content, "返回操作界面", 'account.php?act=account_award','info');
        }
    }
    else if ($surplus['process_type'] == 3)
    {
        ...
        if ($ret == null)
        {
            show_message("您的转现交易失败 ！", "返回操作界面", 'account.php?act=account_goldtransfer','info');
        }
        else if ($ret['resp_code'] === 0) 
        {
            show_message("您的转现交易成功 ！", "返回操作界面", 'account.php?act=account_goldtransfer','info');
        } else {
            $content = '金币转账失败：'.$ret['resp_msg'] ;
            show_message($content, "返回操作界面", 'account.php?act=account_goldtransfer','info');
        }
    }
	...
	else
	{
		...
		{
            show_message("您的转现交易失败 ！", "返回操作界面", 'account.php?act=account_deposit','info');
		    return 0 ;
		}
...
//	$res = $GLOBALS['db']->selectLimit($sql, $num, $start);
	$acctype = 'BAL';
	$res = gc_getaccountlist($user_id,$acctype,$start,$num);
	if ($res)
	{

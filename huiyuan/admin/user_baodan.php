...
if ($_REQUEST['act']=='list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['102_user_baodan']);
    $smarty->assign('full_page',    1);
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_baodan.htm'), '', array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax 删除留言
/*------------------------------------------------------ */
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
           case 'remove':
                $db->query("DELETE FROM " . $ecs->table('baoapp') . " WHERE " . db_create_in($_POST['checkboxes'], 'o_id'));
                 break;
        ...
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'user_baodan.php?act=list');
...
function msg_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('baoapp')." WHERE o_id >0 " . $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    /* 分页大小 */
    $filter = page_and_size($filter);

    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('baoapp') . " WHERE o_id > 0 $where  ORDER by $filter[sort_by] $filter[sort_order] ".

...
$myrealname = $db->getOne("SELECT content FROM " . $ecs->table('reg_extend_info')." WHERE reg_field_id=101 and user_id = '$user_id'");
           if(empty($myrealname))
	       { 
	        $myrealname='';
	       }
...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','check_recode','clear_history','qpassword_name', 'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log','account_list','accountjf_list', 'account_detail','account_detailjf', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'afftree','comment_list','upvip','fhlist','myreg','myrecom','buy_list','account_list','upbaodan','in_register','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
/* 取出大家共用的数字 */ 
   $myinfo = $db->getRow("SELECT user_rank,user_id,user_name,relevel FROM " . $ecs->table('users')." WHERE user_id = '$user_id'");
  $myrank=$myinfo['user_rank'];
  $mylevel=$myinfo['relevel'];
  $user_name=$myinfo['user_name'];
  $daiinfo = $db->getRow("SELECT rank_id,rank_name FROM " . $ecs->table('user_rank')." WHERE rank_id = '$myrank'");
    if(empty($daiinfo))
	 {$maxdai=1000;
	  $urankname='';
	  }
	 ...
	 {
	   $maxdai=100000;
	   $urankname=$daiinfo['rank_name'];
	 }
$urealname = $db->getOne("SELECT content FROM " . $ecs->table('reg_extend_info')." WHERE reg_field_id=101 and user_id = '$user_id'");
    if(empty($urealname))
	 { 
	  $urealname='';
	  }
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other,$recode) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
...
elseif ($action == 'act_register_in')
{
    ...
    else
    {
        ...
        if (registerin($username, $password, $email, $other,$recode) !== false)
        {
            /*把新注册用户的扩展信息插入数据库*/
            $sql = 'SELECT id FROM ' . $ecs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有自定义扩展字段的id
            $fields_arr = $db->getAll($sql);
           //取得新注册的会员的user_id;
			$sqlu ="SELECT user_id  FROM " .  $ecs->table('users') . " where user_name='".$username."'";
           $reguid= $db->getOne($sqlu);
            ...
            foreach ($fields_arr AS $val)
            {
                ...
                {
                    $temp_field_content = strlen($_POST[$extend_field_index]) > 100 ? mb_substr($_POST[$extend_field_index], 0, 99) : $_POST[$extend_field_index];
                    $extend_field_str .= " ('" . $reguid . "', '" . $val['id'] . "', '" . compile_str($temp_field_content) . "'),";
                }
            }
            $extend_field_str = substr($extend_field_str, 0, -1);
            ...
            if (!empty($passwd_answer) && !empty($sel_question))
            {
                $sql = 'UPDATE ' . $ecs->table('users') . " SET `passwd_question`='$sel_question', `passwd_answer`='$passwd_answer'  WHERE `user_id`='" . $reguid . "'";
                $db->query($sql);
            }
            /* 判断是否需要自动发送注册邮件 */
            if ($GLOBALS['_CFG']['member_email_validate'] && $GLOBALS['_CFG']['send_verify_email'])
...
elseif ($action == 'upvip')
{
    ...
    /* 获取用户留言的数量 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('buyorder').
           " WHERE whopay ='$user_id' or  uid = '$user_id' ";
    $record_count = $db->getOne($sql);
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page, 5);
...
elseif ($action == 'upbaodan')
{
    ...
    /* 获取用户报单中心的状态 */
    $sql = "SELECT user_rank FROM " .$ecs->table('users').
           " WHERE  user_id = '$user_id'";
    $ifbao = $db->getOne($sql);
 /* 获取用户成为报单中心的时间 */
    $sql = "SELECT baotime FROM " .$ecs->table('users').
           " WHERE  user_id = '$user_id'";
    $baotime = $db->getOne($sql);
...
/* 获取用户可用资金余额 */
   ...
   $smarty->assign('bd_fee', $_CFG['appfee']);
...
elseif ($action == 'act_add_baodan')
{
    ...
    $message = array(
        'whopay'     => $user_id,
        'paycode'   => $_SESSION['user_name'],
        'bd_fee'    => isset($_POST['bd_fee']) ? intval($_POST['bd_fee'])     : 1,
        'buy_uname'   => $_SESSION['user_name'],
        'bordernum'=>date('YmdHis',time())
            );

    if (add_baodan($message))
    {
...
elseif ($action == 'account_list')
{
	...
	$sqluname = "SELECT user_name FROM " .$ecs->table('users')." WHERE user_id = '$theuid'";
    $theuname = $db->getOne($sqluname);

    $account_type = 'user_money';

    /* 获取记录条数 */
    $sql = "SELECT COUNT(*) FROM " .$ecs->table('account_log').
           " WHERE user_id = '$theuid'" .
           " AND $account_type <> 0 ";
    $record_count = $db->getOne($sql);
    $actionall=$action.'&uid='.$theuid;
    //分页函数
...
elseif ($action == 'accountjf_list')
{
    ...
    //获取剩余余额
    $surplus_amount = get_user_surplusjf($theuid);
    if (empty($surplus_amount))
    {
        $surplus_amount = 0;
    }
...
elseif ($action == 'account_detailjf')
{
    ...
    //获取剩余积分余额
    $surplus_amount = get_user_surplusjf($user_id);
    if (empty($surplus_amount))
    {
        $surplus_amount = 0;
    }
...
elseif ($action == 'account_log')
{
    ...
    //模板赋值
    $smarty->assign('a_amount', $surplus_amount);
    $smarty->assign('surplus_amount', price_format($surplus_amount, false));
    $smarty->assign('account_log',    $account_log);
    $smarty->assign('pager',          $pager);
    $smarty->display('user_transaction.dwt');
...
elseif ($action == 'afftree')
{
   ...
   //$disdai=$myrelevel+$maxdai;
    ...
    /* 获取该会员伞下面的会员信息 */
    $sql = "SELECT user_id,user_name,reid,repath,user_rank " .
            "FROM " .$ecs->table('users') ."where   INSTR(repath,',".$user_id.",')>0 ";
    $li = $db->getAll($sql);
     foreach ($li  as $key=>$val)
        {
            $uid =$li[$key]['user_id'];
			$rankid =$li[$key]['user_rank'];
            $nameinfo = $db->getRow("SELECT rank_id,rank_name FROM " . $ecs->table('user_rank')." WHERE rank_id = '$rankid'");
           if(empty($nameinfo))
	       { 
	        $userrankname='';
	       }
	      ...
	      {
	   	   $userrankname=$nameinfo['rank_name'];
	       }
		   //获取真实姓名
          $realname = $db->getOne("SELECT content FROM " . $ecs->table('reg_extend_info')." WHERE reg_field_id=101 and user_id = '$uid'");
           if(empty($realname))
	       { 
	        $realname='';
	       }
	       $li[$key]['user_rankname']=$userrankname;
           $li[$key]['user_realname']=$realname;
        }

	$my['user_id']=$user_id;
    $my['user_name']=$user_name;
    $my['user_rankname']=$urankname;
    $my['user_realname']=$urealname;
    $smarty->assign('my',$my);
   	$smarty->assign('li',$li);
    $smarty->display('user_clips.dwt');
}

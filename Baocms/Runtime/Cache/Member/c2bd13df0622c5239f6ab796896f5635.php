        ...
        <div class="tgdd_nr">
            <table border="0" cellspacing="0" width="100%"  style=" border-collapse: collapse; margin:0px; vertical-align:middle; text-align:center;">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$var): $mod = ($i % 2 );++$i;?><tr class="tgdd_nrBh">
                        <td colspan="6" style="text-align:left;">对应订单：<span class="tgdd_bh"><?php echo ($var["order_id"]); ?></span></td>
                    </tr>
                    <tr class="tgdd_nrC">
                        <td style="text-align:left;" width="335px">
                            <div class="tgdd_tw">
                                <div class="left"><a target="_blank" href="<?php echo U('pchome/tuan/detail',array('tuan_id'=>$var['tuan_id']));?>"><img src="__ROOT__/attachs/<?php echo ($tuans[$var['tuan_id']]['photo']); ?>" width="81" height="60" /></a></div>
                                <div class="lef"><a class="myInfor_sx" target="_blank" href="<?php echo U('pchome/tuan/detail',array('tuan_id'=>$var['tuan_id']));?>"><?php echo ($tuans[$var['tuan_id']]['title']); ?></a><p>有效期至：<?php echo ($var["fail_date"]); ?></p><p>购买时间：<?php echo (date('Y-m-d',$var["create_time"])); ?></p></div>
                            </div>
                        </td>
                        <td class="tgdd_nrCtd"><?php echo ($var["code"]); ?></td>
                        <td class="tgdd_nrCtd" id="price-color">￥<?php echo round($tuans[$var['tuan_id']]['price']/100,2);?></td>
                         <td class="tgdd_nrCtd" id="price-color">￥<?php echo round($tuans[$var['tuan_id']]['tuan_price']/100,2);?></td>
                        <td class="tgdd_nrCtd" <?php if($var['fail_date'] < $today): ?>style="color:red"<?php endif; ?> ><?php echo ($var["fail_date"]); ?></td>
                    <td class="tgdd_nrCtd"> 
                    ...
                    <td class="tgdd_nrCtd">
                        <?php if($var['status'] == 0): if(($var["is_used"]) == "1"): ?><a mini='confirm' class="myInfor_sx"  href="<?php echo U('index/delete',array('code_id'=>$var['code_id']));?>">删除</a>
                            <?php else: ?>
                                <?php if(($var["price"]) != "0"): ?><a mini='confirm' class="myInfor_sx"  href="<?php echo U('index/coderefund',array('code_id'=>$var['code_id']));?>">申请退款</a>
									
                                <?php else: ?>
                                    到店付<?php endif; endif; ?>    
                        <?php elseif($var['status'] == 1): ?>
                             <a mini='confirm' class="myInfor_sx"  href="<?php echo U('index/codereturn',array('code_id'=>$var['code_id']));?>">取消退款申请</a>
                        <?php else: ?>
                            已完成退款 <a mini='confirm' class="myInfor_sx"  href="<?php echo U('index/delete',array('code_id'=>$var['code_id']));?>">删除</a><?php endif; ?>
                    </td>

    ...
    <div class="tgdd">
        ...
        <div class="tgdd_nr">
            <table border="0" cellspacing="0" width="100%"  style=" border-collapse: collapse; margin:0px; vertical-align:middle; text-align:center;">
                ...
                <?php if(is_array($list)): foreach($list as $key=>$order): ?><tr class="tgdd_nrBh">
                        <td colspan="2" style="text-align: left; text-indent: 20px;">
                            订单号：<?php echo ($order['order_id']); ?>
                            供应：<?php echo ($shops[$order['shop_id']]['shop_name']); ?>
                        </td>
                        <td colspan="4" style="text-align: left; text-indent: 20px;">
                            收货地址：<?php echo ($areas[$addrs[$order['addr_id']]['area_id']]['area_name']); ?>、
                    ...
                    <td width="80">
                        <?php if($order['status']==0): ?><a mini='confirm' class="tgdd_del" href="<?php echo U('order/goodsdel',array('order_id'=>$order['order_id']));?>">
                                <font color="red">取消订单</font>
                            </a><?php endif; ?>
                    <?php if($order['status']==2): ?><a mini='confirm' class="tgdd_del" href="<?php echo U('order/goodsshou',array('order_id'=>$order['order_id']));?>">
                                <font color="red">确认已收货</font>
                            </a><?php endif; ?>
                    </td>
                    </tr>
                    <?php if(is_array($shops)): foreach($shops as $key=>$shop): if(is_array($goods)): $index = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($index % 2 );++$index; if(($good['shop_id'] == $shop['shop_id']) AND ($good['order_id'] == $order['order_id'])): ?><tr class="tgdd_nrC">
                                    <td class="tgdd_nrCtd"><img style="margin: 0px auto;" src="__ROOT__/attachs/<?php echo (($good['photo'])?($good['photo']):'default.jpg'); ?>" width="100" height="75"/></td>
                                    <td colspan="2" class="tgdd_nrCtd"><?php echo ($good['title']); ?> <?php echo ($good["spec_name"]); ?></td>
                                    <td class="tgdd_nrCtd" id="price-color">￥<?php echo round($good['price']/100,2);?></td>
                                    <td class="tgdd_nrCtd"><?php echo ($good["num"]); ?></td>
                                    <td class="tgdd_nrCtd" id="price-color">￥<?php echo round($good['total_price']/100,2);?></td>
                                    <td class="tgdd_nrCtd">&nbsp;</td>
                                    <td class="tgdd_nrCtd">&nbsp;</td>
                                </tr><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; endforeach; endif; ?>
            </table>

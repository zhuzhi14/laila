...
<script>
    $(document).ready(function () {
        $('.zy_doorSer_flexslider').flexslider({
            directionNav: true,
            pauseOnAction: false
        });
    });//上门服务首页轮播
</script>
<div class="pagewd zy_doorSer">
    <div class="zy_doorSer_banner mt10">
        <div class="left">
            <div class="zy_doorSer_flexslider">
                <ul class="slides">
                    <?php  $cache = cache(array('type'=>'File','expire'=> 7200)); $token = md5("Ad, closed=0 AND site_id=4 AND city_id IN ({$city_ids}) and bg_date <= '{$today}' AND end_date >= '{$today}' ,0,3,7200,orderby asc,,"); if(!$items= $cache->get($token)){ $items = D("Ad")->where(" closed=0 AND site_id=4 AND city_id IN ({$city_ids}) and bg_date <= '{$today}' AND end_date >= '{$today}' ")->order("orderby asc")->limit("0,3")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li><a target="_blank" href="<?php echo ($item["link_url"]); ?>"><img src="__ROOT__/attachs/<?php echo ($item["photo"]); ?>" width="960" height="360" /></a></li> <?php endforeach; ?>
                </ul>
            </div>
        </div>
        <div class="right">
            <div class="img"><img src="__ROOT__/attachs/<?php echo ($CONFIG["site"]["wxcode"]); ?>" width="156" height="156" /></div>
            <p>扫一扫，下载APP</p>
            <img src="__TMPL__statics/images/doorSer_wz.png" width="142" height="23" />
        </div>
        <div class="clear"></div>
    </div>
    <h2 class="mt10">上门家政</h2>
    <div class="sy_doorSer_list">
        <ul>
            ...
            <?php if(is_array($list)): foreach($list as $key=>$item): ?><li>
                    ...
                    <div class="btn_box"><a href="<?php echo U('lifeservice/detail',array('id'=>$a));?>">立即预约</a></div>
                </li>

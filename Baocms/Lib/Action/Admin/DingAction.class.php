...
class DingAction extends CommonAction {
    ...
    public function create() {
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                foreach($thumb as $k=>$val){
                        D('Shopdingpics')->add(array('shop_id'=>$data['shop_id'],'photo'=>$val));
                    }
                    foreach($type as $k=>$val){
                        D('Shopdingattr')->add(array('shop_id'=>$data['shop_id'],'type_id'=>$val));
                    }
        ...
        if ($shop_id = (int) $shop_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    D('Shopdingpics')->where(array('shop_id'=>$shop_id))->delete();
                    foreach($thumb as $k=>$val){
                        D('Shopdingpics')->add(array('shop_id'=>$shop_id,'photo'=>$val));
                    }
                    D('Shopdingattr')->where(array('shop_id'=>$shop_id))->delete();
                    foreach($type as $k=>$val){
                        D('Shopdingattr')->add(array('shop_id'=>$shop_id,'type_id'=>$val));
                    }
                    $this->baoSuccess('操作成功', U('ding/index'));
                }
                $this->baoError('操作失败');
                ...
                $thumb = D('Shopdingpics')->where(array('shop_id'=>$shop_id))->select();
                $this->assign('thumb', $thumb);
                $have_type = D('Shopdingattr')->where(array('shop_id'=>$shop_id))->select();
                $typess = array();
